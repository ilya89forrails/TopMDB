...
describe 'Movie_list' do
  ...
  describe :count_by_actors do
    context 'count movies by every actor from list' do
      ...
      it do
        is_expected.to eql([['Aaron Eckhart', 1], ['Cathy Moriarty', 1], ['Christian Bale', 1], ['Heath Ledger', 1],
  ...
  describe :filter do
    context 'adds filters and filter by it' do
      before(:each) do
        ...
        movies.add_filter(:years) { |movie, from, to| (from..to).cover?(movie.year.to_i) }
      end
  ...
  describe :genres_list do
    context 'shows list of unique genres' do
      ...
      it { is_expected.to eql(%w(Action Crime Drama Biography Sport)) }
    end
  end
end
