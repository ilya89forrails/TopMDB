...
module TopMDB
  MOVIE_KEYS = [:link, :title, :year, :country, :release_date, :genre, :length, :rating, :editor, :actors].freeze
  ...
  class MoviesList
    ...
    def by_genre(genre)
      @movies.sort_by { |movie| movie.release_date unless movie.release_date.nil? }
    ...
    def by_day
      @movies.collect { |movie| movie.release_date.day unless movie.release_date.nil? }
             .group_by { |i| i }.collect { |e, group| [e, group.count] }.to_h
             .delete_if { |key, _value| key.nil? }.sort
             .collect { |a, b| (a.to_s + ' => ' + b.to_s) }
    end

    def by_month
      @movies.collect { |movie| movie.release_date.mon unless movie.release_date.nil? }
             .group_by { |i| i }.collect { |e, group| [e, group.count] }.to_h
             .delete_if { |key, _value| key.nil? }.sort
    ...
    def print(&block)
      block = proc { |m| m.to_s } unless block_given?
